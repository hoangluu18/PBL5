...
public interface OrderDetailRepository extends JpaRepository<OrderDetail, Integer> {
    ...
    @Query("SELECT od FROM OrderDetail od WHERE od.order.id = ?1 AND od.order.customer.id = ?2")
    ...
    List<OrderDetail> findByProductIdAndCustomerIdWithStatusDelivered(Integer productId, Integer customerId);
}